...
class DmSimulatorPy(BaseBackend):
    ...
    @nvtx.("_initialize_densitymatrix(self)",color="black")
...
#                                                qubit0, qubit1, self._error_params['two_qubit_gates'],self._number_of_qubits)
    @nvtx.annotate("_add_unitary()",color="blue")
    def _add_unitary(self, gate, qubits):
        num_qubits=len(qubits)
        if num_qubits == 1:
            lt, mt, rt = 4 ** qubits[0], 4, 4 ** (self._number_of_qubits-qubits[0]-1)
            self._densitymatrix = np.reshape(self._densitymatrix, (lt, mt, rt))

            for idx in gate: # For Rotations in the Decomposed Gate list
                self._densitymatrix = rot_gate_dm_matrix(
                    idx[0], idx[1], self._error_params['one_qubit_gates'][idx[0]], self._densitymatrix, qubits[0], self._number_of_qubits)
            self._densitymatrix = np.reshape(self._densitymatrix,
                                        self._number_of_qubits * [4])

        elif num_qubits == 2:
            self._densitymatrix = cx_gate_dm_matrix(self._densitymatrix,
                                                    qubits[0], qubits[1], self._error_params['two_qubit_gates'],self._number_of_qubits)
        else:
            pass
    @nvtx.annotate("_add_decoherence_and_amp_decay()",color="red")
    ...
    def _run_job(self, job_id, qobj):
        ...
        return result
    @nvtx.annotate("run_experiment()",color="yellow")
...
#        print(type(partitioned_instructions))
#        print(len(partitioned_instructions))
#        for current in range(len(partitioned_instructions)):
#            print("\nPartitioned_Instructions ",type(current))
    ...
    @nvtx.annotate(“_compute_densitymatrix()”, color="purple")
...
#            print("\nType of Current  Partition ", type(current_partition))
